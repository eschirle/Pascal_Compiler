...
//--------------------------//
    ...
    case class Procedure (nm: String, p: List[(Var, Typ)], body: Stmt)              extends Func        //I can parse procedures but cannot compile. 
...
//--------------------------//
    ...
    def compile (prog : Program, filename: String) : Unit = {
        ...
        val vars : List[String] = for(vdec <- prog.vd) yield { getName(vdec) }          //set up env with declared global variables. 
...
// MAIN
    def main (args : Array[String]) {
        ...
        testComp(MyParsers.prgm, "input/ch03/test.p")
        println ("=" * 80)

        testComp(MyParsers.prgm, "input/ch03/test2.p")
        println ("=" * 80)

        testComp(MyParsers.prgm, "input/ch03/print1-100.p")
        println ("=" * 80)

        testComp(MyParsers.prgm, "input/ch03/fact1-20.p")
        println ("=" * 80)

        testComp(MyParsers.prgm, "input/ch03/notDiv3-5.p")
        println ("=" * 80)
    }
